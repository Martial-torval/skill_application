...
class SecurityController extends AbstractController
{
    ...
    public function examens(ExamensRepository $repo, Request $request, ManagerRegistry $doctrine): Response
    {   
        $entityManager = $doctrine->getMana
        $examens = $repo->findAll();
        $isRegistered = new Inscription();
        $isRegistered->setUser($request->request->get('user_id'))
                     ->setExamens($request->$request->get('examen_id'));

        $entityManager->persist($isRegistered);
        $entityManager->flush();
