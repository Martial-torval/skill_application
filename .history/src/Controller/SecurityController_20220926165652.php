...
public function show($id, ExamensRepository $repo, InscriptionRepo $repo2) {
//    $id = $_POST['id'];
    $inscription = new Inscription();
    $user = $this->getUser();
    $examens = $repo->find($id);
    $repo2->add($user);
    $repo->add($examens, true);
...
return $this->render('session/session.html.twig',['examens' => $examens, 'users' =>$user]);
}
